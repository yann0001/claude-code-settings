...
const BRAINSTORM_VERSION = readBrainstormVersion();
...
function readBrainstormVersion() {
  return '1.0.0';
}
...
function brandMarkup() {
  const version = escapeHtmlText(BRAINSTORM_VERSION);
  return '<div class="brand"><span class="brand-copy">Brainstorming v' + version + '</span></div>';
}
